...
declare
  ...
  ts_manana timestamptz := manana::timestamptz at time zone 'America/Mexico_City';
  ts_hoy timestamptz := hoy::timestamptz at time zone 'America/Mexico_City';

  -- helper: evita duplicados por (user_id, origen_tabla, origen_id, not enviada)
  procedure insertar_si_no_existe(
    p_user_id uuid,
    p_origen_tabla text,
    p_origen_id uuid,
    p_mensaje text,
...
begin
  -- Regla 1: pendientes con fecha_limite = mañana
  for r in
    select id, user_id, titulo from public.pendientes
    where hecho = false and fecha_limite = manana
  loop
    call insertar_si_no_existe(r.user_id, 'pendientes', r.id,
      'Vence mañana: ' || r.titulo, ts_manana);
  end loop;

  -- Regla 2: cobranzas vencidas hoy (pendiente o parcial)
  for r in
    select id, user_id, deudor, concepto from public.cobranzas
    where estado in ('pendiente','parcial') and fecha_limite <= hoy
  loop
    call insertar_si_no_existe(r.user_id, 'cobranzas', r.id,
      'Cobranza vencida: ' || r.deudor || ' — ' || r.concepto, ts_hoy);
  end loop;

  -- Regla 3: pagos_programados a 2 días
  for r in
    select id, user_id, acreedor, monto from public.pagos_programados
    where estado = 'pendiente' and fecha_limite = en_dos
  loop
    call insertar_si_no_existe(r.user_id, 'pagos_programados', r.id,
      'Pago próximo: ' || r.acreedor || ' $' || r.monto::text, ts_manana);
  end loop;

  -- Regla 4: eventos de mañana
  for r in
    select id, user_id, titulo, hora, tipo from public.eventos
    where fecha = manana
  loop
    call insertar_si_no_existe(r.user_id, 'eventos', r.id,
      'Mañana: ' || r.titulo || coalesce(' ' || left(r.hora::text, 5), ''), ts_manana);
    -- Entrevistas: alerta adicional 2h antes el mismo día
    if r.tipo = 'entrevista' and r.hora is not null then
      call insertar_si_no_existe(r.user_id, 'eventos_2h', r.id,
        '2h para tu entrevista: ' || r.titulo,
        (manana::timestamptz at time zone 'America/Mexico_City')
          + (r.hora::interval) - interval '2 hours');
    end if;
  end loop;
  ...
  loop
    declare
      ts_toma timestamptz;
    begin
      ts_toma := (hoy::text || ' ' || r.horario)::timestamptz at time zone 'America/Mexico_City';
      if not exists (
        select 1 from public.alertas a
  ...
  -- Regla 6: predicción de ciclo a 2 días (promedio de intervalos entre registros)
    ...
    begin
      ...
      if prom_dias is not null and ultimo_inicio is not null then
        ...
        if proximo = en_dos then
          call insertar_si_no_existe(uid, 'ciclo', null,
            'Recordatorio de calendario personal',
            (en_dos::timestamptz at time zone 'America/Mexico_City') + interval '8 hours');
        end if;
      end if;
    end;
  end loop;

  -- Regla 7: suplementos con recompra_fecha <= hoy
  for r in
    select id, user_id, nombre from public.suplementos
    where activo = true and recompra_fecha <= hoy
  loop
    call insertar_si_no_existe(r.user_id, 'suplementos', r.id,
      'Recomprar: ' || r.nombre, ts_hoy);
  end loop;
    ...
    begin
      ...
      if ultima_alerta_chekin is null or ultima_alerta_chekin < hoy - 3 then
        ...
        if (ultimo_sueno is null or ultimo_sueno < hoy - 3)
          ...
          call insertar_si_no_existe(uid, 'checkin', null,
...
-- ── pg_cron (correr DESPUÉS de habilitar la extensión pg_cron en el Dashboard) ──
